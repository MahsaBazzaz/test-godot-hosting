    ...
    for (let row = 0; row < rows.length; row++) {
        for (let col = 0; col < rows[row].length; col++) {
            ...
            if (char == "X" && y != this.Height - 1){
                ...
                if (rows[row+1][col] == "<" || rows[row+1][col] == ">") {
            ...
            level.SetBlock(x, y, 0 * 16 + 0);
            }
            else if (char == "}"){
                level.ExitX = x;
                level.ExitY = this.Height - 1;
            }
        }
    }
    
    // 2) Draw flat ground on the bottom row

    // var floorY = this.Height - 1;
    // for (var x = 0; x < this.Width; x++) {
    //     level.SetBlock(x, floorY, 11 * 16 + 5);   
    // }
    
    // // 3) Carve a gap at x=20 (no ground there)
    // var gapX = 20;
    // level.SetBlock(gapX, floorY, 0);
    
    // // 4) Simple 2-wide pipe at x=10
    // var pipeX   = 10,
    //     pipeTop = floorY - 2;
    // // pipe tops
    // level.SetBlock(pipeX,     pipeTop,     10);
    // level.SetBlock(pipeX + 1, pipeTop,     11);
    // // pipe stems
    // level.SetBlock(pipeX,     pipeTop+1,   10 + 16);
    // level.SetBlock(pipeX + 1, pipeTop+1,   11 + 16);
    
    // // 5) Place a Goomba and a Koopa near the pipe
    // //    (x, y, SpriteTemplate(enemyType, movesLeft?))
    // level.SetSpriteTemplate(pipeX + 3, floorY - 1, new Mario.SpriteTemplate(Mario.Enemy.Goomba, false));
    // level.SetSpriteTemplate(pipeX + 6, floorY - 1, new Mario.SpriteTemplate(Mario.Enemy.Koopa, false));
    
    // // 6) Sprinkle coins in the air above the gap
    // //    (assuming block ID 5 is the "coin block" — swap to your engine’s coin tile)
    // var coinTile = 5;
    // for (var i = -2; i <= 2; i += 2) {
    //     level.SetBlock(gapX + i, floorY - 3, coinTile);
    // }
